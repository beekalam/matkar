...
class Cart extends BASE_REST_Controller
{
    ...
    public function lines()
    {
        ...
        if ($res["order"]) {
            $res["lines"] = $this->db->select('order_lines.id as line_id,order_lines.quantity,order_lines.product_id')
                                     ->select('products.title,products.description,products.price_per_unit')
                                     ->select("units.unit_name")
                                     ->select("CONCAT(resellers.reseller_firstname,' ',resellers.reseller_lastname) as reseller_name")
                                     ->select("(quantity * price_per_unit) as line_price")
                                     ->from("order_lines")
            ...
            $res["order"]["total_price"] = array_sum(array_column($res["lines"], "line_price"));
        } else {
    ...
    public function add_line()
    {
        ...
        if ($order) {
            // update order line if not already in basket
            $where      = array(
                "order_id"   => $order["id"],
